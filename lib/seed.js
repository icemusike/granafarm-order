...
// Fotografii reale per soi de roșii (fișiere încărcate în /images/products/).
// Cheile sunt fragmente normalizate (fără diacritice) din numele produsului.
// Notă: „Roz Dov" folosește temporar fotografia Roz Rose până la încărcarea
// unui fișier dedicat (roz_dov.png).
...
// Imaginile generice de familie pot fi înlocuite automat cu fotografia de soi;
// o imagine setată manual de administrator nu se atinge.
...
function varietyImageFor(name, category = 'Roșii') {
  // Doar produsele din familia „Roșii" primesc fotografii de soi — altfel
  // „Vinete de grădină" (Legume) s-ar potrivi greșit cu „De Grădină".
  if (category !== 'Roșii') return '';
  const n = normalizeProductName(name);
  const hit = VARIETY_IMAGES.find(([re]) => re.test(n));
  return hit ? hit[1] : '';
}
