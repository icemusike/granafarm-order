...
const DEFAULT_SETTINGS = {
  ...
  // Integrare WhatsApp (Twilio), SEPARATĂ de SMS: propriul cont/expeditor
  // aprobat pentru WhatsApp, propriul comutator (implicit oprit = mod
  // simulat), propriile șabloane cu diacritice.
  whatsapp: {
    enabled: false,
    accountSid: '',
    authToken: '',
    // Expeditorul WhatsApp aprobat în Twilio (numărul, cu sau fără
    // prefixul whatsapp:).
    fromNumber: '',
    // Trimite clientului mesaj WhatsApp când comanda este confirmată.
    confirmations: true,
    // Token-uri șabloane: {number} {name} {company} {total} {city} {phone}
    // {deliveryDate} {trackingUrl} pentru confirmare; {name} {deliveryDay}
    // {deliveryDate} {cutoff} {url} pentru reamintirea de livrare.
    templates: {
      clientConfirmed:
        'GranaFarm: Comanda dvs. {number} în valoare de {total} lei a fost confirmată.{deliveryDate} Vă mulțumim! 🍅',
      deliveryReminder:
        'Bună {name}! 🍅 GranaFarm livrează {deliveryDay}, {deliveryDate}. Trimiteți comanda până azi la {cutoff} pe {url} și o primiți direct la ușă. Vă mulțumim!',
    },
    // Reamintire programată: în zilele bifate (0=duminică .. 6=sâmbătă), la
    // ora setată (ora României), toți clienții din CRM primesc mesajul cu
    // următoarea zi de livrare. lastSentDate previne trimiterea dublă.
    reminder: {
      enabled: false,
      days: [],
      time: '18:00',
  ...
  // e-Factura (ANAF SPV): integrare construită „în spate", NEACTIVATĂ.
  // Cât timp enabled este false, nu se trimite nimic către ANAF; XML-ul
  // UBL (CIUS-RO) se poate totuși genera și descărca din Facturi, pentru
  // verificare. Se activează doar după configurarea contului SPV.
  efactura: {
    enabled: false,
    environment: 'test',
    clientId: '',
    clientSecret: '',
    // Callback URL trebuie să fie identic cu cel înregistrat în portalul
    // ANAF; gol = se folosește adresa publică a aplicației +
    // /api/admin/efactura/callback.
    redirectUri: '',
    // Token-urile se completează automat după „Autorizează cu ANAF"
    // (sau manual, dacă token-ul a fost obținut în altă parte).
    accessToken: '',
    refreshToken: '',
    tokenExpiresAt: '',
    tokenObtainedAt: '',
  ...
  ordering: {
    ...
    // Comenzile se pot plasa toată ziua (până la 24:00); livrăm 7 zile
...
function mergeSettings(stored = {}) {
  ...
  return {
    ...
    whatsapp: {
      ...
      ...(saved.whatsapp || {}),
      templates: { ...DEFAULT_SETTINGS.whatsapp.templates, ...((saved.whatsapp || {}).templates || {}) },
      reminder: { ...DEFAULT_SETTINGS.whatsapp.reminder, ...((saved.whatsapp || {}).reminder || {}) },
    },
    efactura: { ...DEFAULT_SETTINGS.efactura, ...(saved.efactura || {}) },
    ...
    ordering: {
      ...
      // Vechile valori implicite (cutoff 14:00 / 22:00) au putut fi persistate
      ...
      // 7 zile din 7, comenzi până la 24:00).
      cutoffTime: savedOrdering.cutoffTime
        && !['14:00', '22:00'].includes(savedOrdering.cutoffTime)
...
const SETTINGS_SCHEMA = {
  ...
  vatRate: 'number',
  twilio: 'object', whatsapp: 'object', efactura: 'object', maps: 'object', postmark: 'object', marketing: 'object',
