...
function createJsonStorage(dataDir) {
  ...
  async function createOrder({
    ...
    createdAt, status = 'noua', payment = null,
  }) {
    const order = {
  ...
  async function getInvoice(id) {
    const invoice = db.invoices.find((x) => x.id === id);
  ...
  async function listClientData() { return clone(db.clients); }
  async function saveClientData(key, data) {
    db.clients[key] = { ...(db.clients[key] || {}), ...data, updatedAt: new Date().toISOString() };
    save();
    return clone(db.clients[key]);
  }
