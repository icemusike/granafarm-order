...
function createPostgresStorage(connectionString) {
  ...
  // --- comenzi ---
  async function createOrder({ customer, items, subtotal, deliveryFee, total, delivery, trackingHash }) {
    const id = crypto.randomUUID();
    const { rows } = await pool.query(
      `INSERT INTO orders (
         id, number, status, customer, items, subtotal, delivery_fee, total, delivery,
         tracking_token_hash
       ) VALUES (
         $1, 'CMD-' || lpad(nextval('order_number_seq')::text, 4, '0'), 'noua',
         $2, $3, $4, $5, $6, $7, $8
       )
       RETURNING *`,
      [id, JSON.stringify(customer), JSON.stringify(items), subtotal, deliveryFee, total,
       JSON.stringify(delivery || {}), trackingHash]
    );
  ...
  return {
    ...
    createOrder, listOrders, getOrder, getOrderByTrackingHash, setOrderStatus,
